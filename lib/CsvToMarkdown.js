...
//#region node_modules/csv-walker/dist/index.js
/*!
	* csv-walker
	...
	*/
	function option(name, value, empty = false) {
		if (empty && value === "" || [...value].length === 1) return (settings) => {
			settings[name] = value;
		};
	...
	*/
	...
	function csv(settings) {
		...
		function readQuotedColumn(character) {
			if (character === settings.enclosure) return readQuote;
	...
	function nextCell(reader, input) {
		while (true) {
			...
			if (character.done) return reader.finish();
	...
	function* columns(first, reader, input, complete) {
		let cell = first;
		while (true) {
			if (cell.last) complete.value = true;
			yield cell.value;
			if (cell.last) return;
			const next = nextCell(reader, input);
			if (!next) throw new Error("CSV ended before the record did");
			cell = next;
		}
	}
	...
	function configure(options) {
		...
		for (const apply of options) apply(value);
		return value;
	}
	function* parseString(text, options) {
		...
		while (true) {
			...
			if (!complete.value) skipRow(reader, input);
		}
	}
	...
	async function* characters(source, label) {
		...
		for await (const chunk of chunks(source)) {
			const text = typeof chunk === "string" ? `${(_a = decoder === null || decoder === void 0 ? void 0 : decoder.decode()) !== null && _a !== void 0 ? _a : ""}${chunk}` : (decoder !== null && decoder !== void 0 ? decoder : decoder = new TextDecoder(label)).decode(chunk, { stream: true });
			if (typeof chunk === "string") decoder = void 0;
			for (const character of text) yield character;
		}
		if (decoder) for (const character of decoder.decode()) yield character;
	}
	async function* parseSource(source, options) {
		...
		try {
			while (true) {
				...
				if (!complete.value) await skipAsyncRow(reader, input);
			}
		} finally {
			if (input.return) await input.return(void 0);
		}
	}
	...
	function allValues(values) {
		const result = [];
		if (Symbol.asyncIterator in values) return (async () => {
			for await (const value of values) result.push(typeof value === "string" ? value : await allValues(value));
			return result;
		})();
		for (const value of values) result.push(typeof value === "string" ? value : [...value]);
		return result;
	}
